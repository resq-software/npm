...
This directory contains the project's git hooks. They enforce code quality, security, and workflow conventions for the ResQ UI component library (TypeScript/Bun/Biome).

> **Note on Husky:** The `.husky/` directory is retained for compatibility with `bun install --prepare`. When `core.hooksPath` is set to `.git-hooks` (via `./scripts/setup.sh`), Git uses these hooks instead. The Husky directory is not deleted.
...
| Hook | Purpose |
|------|---------|
| `pre-commit` | Large file guard (1 MB limit), secrets scan (gitleaks / grep fallback), Biome auto-format + re-stage `.ts`/`.tsx`/`.js`/`.jsx`/`.json`/`.css` files, copyright header generation via `scripts/copyright.gen.ts` |
| `commit-msg` | Conventional Commits format validation; blocks `fixup!`/`squash!`/WIP on `master` |
| `prepare-commit-msg` | Prepends ticket reference (e.g., `[PROJ-123]`) extracted from branch name |
| `pre-push` | Force-push guard on `master`, branch naming convention, Biome lint check |
| `post-checkout` | Auto `bun install` when `bun.lockb` changes between branches |
| `post-merge` | Auto `bun install` when `bun.lockb` changes after a merge |
