...
describe("FetcherValidationError", () => {
	it("stores url, problems, responseData, and attempt", () => {
		const error = new FetcherValidationError(
			"invalid",
			"/api",
			"id: expected number",
			{ id: "x" },
...
describe("fetcher – POST", () => {
	...
	it("sends a POST with text body", async () => {
		...
		const result = await Effect.runPromise(
			Effect.provide(
				fetcher("/api/text", "POST", { bodyType: "text" }, undefined, "raw text"),
				clientLayer,
			),
		);
...
describe("HTTP method helpers", () => {
	...
	it("del sends DELETE method", async () => {
		...
		const result = await Effect.runPromise(Effect.provide(del("/api/items/1"), clientLayer));
	...
	it("options sends OPTIONS method", async () => {
		const clientLayer = makeClient((req) => {
			...
			return Effect.succeed(
				HttpClientResponse.fromWeb(
					req,
					new Response(JSON.stringify({ allowed: ["GET", "POST"] }), { status: 200 }),
				),
			);
		});
...
describe("fetcher – HTTP errors", () => {
	it("returns FetcherError on 404", async () => {
		const clientLayer = makeClient((req) =>
			Effect.succeed(HttpClientResponse.fromWeb(req, new Response("Not Found", { status: 404 }))),
		);

		const error = await Effect.runPromise(
			Effect.flip(Effect.provide(fetcher("/api/missing"), clientLayer)),
		);
	...
	it("returns FetcherError on 500", async () => {
		...
		const error = await Effect.runPromise(
			Effect.flip(Effect.provide(fetcher("/api/error"), clientLayer)),
		);
		expect(error).toBeInstanceOf(FetcherError);
		const fetcherErr = error as FetcherError;
	...
	it("returns FetcherError with rate limit message on 429", async () => {
		const clientLayer = makeClient((req) =>
			Effect.succeed(HttpClientResponse.fromWeb(req, new Response("Too Many", { status: 429 }))),
		);

		const error = await Effect.runPromise(
			Effect.flip(Effect.provide(fetcher("/api/rate"), clientLayer)),
		);
		expect(error).toBeInstanceOf(FetcherError);
		const fetcherErr = error as FetcherError;
	...
	it("returns FetcherError on network failure", async () => {
		...
		const error = await Effect.runPromise(
			Effect.flip(Effect.provide(fetcher("/api/dead"), clientLayer)),
		);
		expect(error).toBeInstanceOf(FetcherError);
		const fetcherErr = error as FetcherError;
...
describe("fetcher – retries", () => {
	...
	it("does NOT retry on 400 (client error)", async () => {
		...
		const error = await Effect.runPromise(
			Effect.flip(
				Effect.provide(fetcher("/api/bad", "GET", { retries: 2, retryDelay: 10 }), clientLayer),
			),
		);
		expect(error).toBeInstanceOf(FetcherError);
...
describe("fetcher – URL construction", () => {
	it("uses absolute URL when input starts with http", async () => {
		...
		await Effect.runPromise(Effect.provide(fetcher("https://example.com/api/data"), clientLayer));
	});
