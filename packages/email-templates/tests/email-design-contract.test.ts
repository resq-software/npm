...
import { renderEmailContractIntegrityModule } from "../scripts/email-contract-integrity-module";
...
describe("email design contract", () => {
	...
	it("pins the schema-3 contract integrity digest", () => {
		expect(emailDesignContractIntegrity).toBe(
			"0f5e172c3ef8d2f6231a4e20c5a34fe59fe8be1d3ba61558b0c8660f327581f0",
		);
	});
	...
	it("keeps the checked-in integrity module byte-for-byte deterministic", () => {
		...
		expect(generatedSource).toBe(renderEmailContractIntegrityModule(emailDesignContractIntegrity));
	});
