...
describe("formatDatePeriod", () => {
	...
	test('returns "Present" when isCurrent is true', () => {
		expect(formatDatePeriod("2023-01-01T00:00:00Z", null, true)).toBe("Jan 2023 - Present");
	});

	test('returns "Present" when endDate is null and isCurrent is false', () => {
		expect(formatDatePeriod("2023-06-01T00:00:00Z", null)).toBe("Jun 2023 - Present");
	});

	test('returns "Present" when endDate is undefined', () => {
		expect(formatDatePeriod("2023-06-01T00:00:00Z")).toBe("Jun 2023 - Present");
	});
