 ...
 *   or `process.env.NEXT_PUBLIC_GA4_ID`. Accepts `undefined` for
 *   convenience so call sites don't need a guard.
 * @returns The validated ID when it matches Google's format, otherwise
 *   `null`. Skip GA4 init entirely when this returns `null`.
 */
export function sanitizeGa4Id(id: string | undefined): string | null {
	if (!id) return null;
 ...
 *   `window.location.hostname`. Pass `undefined` (or call without an
 *   argument) on the server so this returns `undefined` cleanly.
 ...
 */
export function resolveResqCookieDomain(host?: string | undefined): string | undefined {
