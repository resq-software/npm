...
class JSDocGenerator {
	...
	async processFiles(
		...
		const results = await Promise.all(
			filePaths.map((filePath) =>
				this.processFile(filePath, undefined, overwrite),
			),
		);
		...
		console.log(
			`⚡ Average time per file: ${Math.round(totalDuration / results.length)}ms`,
		);
	...
	async scanGitFiles(
		...
		try {
			...
			const allFiles = gitOutput
				...
				.map((file) =>
					directoryPath === "." ? file : join(directoryPath, file),
				);
	...
	updateConfig(newConfig: Partial<GenerationConfig>): void {
		if (
			...
			newConfig.maxConcurrency !== this.config.maxConcurrency
		) {
			throw new Error(
